...
/**
   ...
   private static class CompareFiles
   {
       ...
       * 
       * @param arg
       *           : the first/third string in the command line which helps us
       *           identify if the path provided is a file or a directory.
       * @param path
       *           : the path corresponding to the arg.
       * @return : a File[] array that contains all the files present
       *         corresponding to the given path, if a directory path is given,
       *         then all the files(success only if the file is .wav or .mp3)
       *         are stored in an array and if its a file then only 1 file is
       *         stored in the array (after checking its format and header)
       */
       ...
       * @return : checks if the path corresponds to a directory, if yes gets
         ...
         for (int path1Count = 0; path1Count < NoOfFilesInPath1; path1Count++)
         {
            ...
            for (int path2Count = 0; path2Count < NoOfFilesInPath2; path2Count++)
            {
               ...
               if (Utilities.DEBUG_MODE_ON)
               {
                  ...
                  System.out.println(rightNow1.get(Calendar.MINUTE) + ":"
                        + rightNow1.get(Calendar.SECOND));
               }
            }
         }
      ...
      {
         AudioProcessableFile f = dirMap.get(fileToProcess.getPath());
         if (f == null)
         {
